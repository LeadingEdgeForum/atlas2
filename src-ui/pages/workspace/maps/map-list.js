...
export default class MapList extends React.Component {
  ...
  render() {
    ...
    if (this.state && this.state.workspace && this.state.workspace.maps && Array.isArray(this.state.workspace.maps)) {
      _mapsToShow = this.state.workspace.maps.map(item => <MapListElement workspaceID={workspaceID} key={item._id} id={item._id} user={item.user} purpose={item.purpose} name={item.name} isSubmap={item.isSubmap} responsible={item.responsiblePerson}></MapListElement>);
